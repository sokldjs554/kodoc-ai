...
학습 자체는 GPU가 필요해 Kaggle 노트북에서 수행한다. 절차는 저장소 외부 런북 참조.
...
# 2) 평가 (OpenAI 호환 엔드포인트면 무엇이든)
python finetune/eval_format.py --model <모델명> --label base --out finetune/results/base.json
```
