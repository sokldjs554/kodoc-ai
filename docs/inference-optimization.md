...
**실측으로 확인한 것과 수정한 것** ([serving-benchmark.md](serving-benchmark.md)):
(a)는 확인됐다 — T4에서 AWQ의 배치 1 TPOT 개선이 2.91배로, 읽는 바이트 감소분에
거의 정확히 비례했다. 다만 **이 이득은 동시성이 오를수록 줄어든다**(2.91 → 1.30배).
가중치 읽기는 배치와 무관한 고정 비용이라 배치가 커질수록 비중이 작아지기 때문이다.
"양자화 = 항상 N배 빠름"이 아니라 **저동시성 구간에 효과가 집중되는 최적화**로
이해해야 한다. (b)는 이 실험에서 검증하지 못했다 — 시퀀스가 짧아 양쪽 모두 KV
여유가 충분했다.
